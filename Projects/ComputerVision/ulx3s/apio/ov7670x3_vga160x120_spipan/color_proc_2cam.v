...
//
//   color_proc_2cam.v
//   - Takes an image from two memories,
//   - Applies a color filter to both images
//   - Saves the processed images into two new memories
//   - Outputs the proximity and the centroid of the detected object
//     considering the combined image of the two memories
//
//   The two images come from two cameras should barely overlap, so the make up
//   a new image
//
//   inputs:
//   ---------
...
module color_proc_1cam
  # (parameter
      ...
      c_nb_centroid = 8,
      c_nb_centr_cmb = 2*c_nb_centroid,
  ...
  (
    ...
    input        new_frame_l, // a new frame from left camera has just ended
    input        new_frame_r, // a new frame from right camera has just ended
    // Address and pixel of original image
    input  [c_nb_buf-1:0]      orig_l_pxl,  //pixel from original left image
    output [c_nb_img_pxls-1:0] orig_l_addr, //pixel mem address original left img
    input  [c_nb_buf-1:0]      orig_r_pxl,  //pixel from original right image
    output [c_nb_img_pxls-1:0] orig_r_addr, //pixel mem address original right img
    // Address and pixel of processed image
    output reg                 proc_l_we, //write enable processed pxl left img
    output [c_nb_buf-1:0]      proc_l_pxl,//proc pxl of left image to be written
    output [c_nb_img_pxls-1:0] proc_l_addr,//address of proc pxl of left image
    output reg                 proc_r_we, //write enable proc pxl right img
    output [c_nb_buf-1:0]      proc_r_pxl,//proc pxl of right img to be written
    output [c_nb_img_pxls-1:0] proc_r_addr,//address of proc pxl of right image
    output reg [c_nb_centroid-1:0] centroid_l, // left centroid (to test)
    output reg [c_nb_centroid-1:0] centroid_r, // right centroid (to test)
    output reg [c_nb_cent_cmb-1:0] centroid, // combined centroid
    output reg new_centroid_l,                 // new left centroid
    output reg new_centroid_r,                 // new right centroid
    output reg new_centroid,                   // new combined centroid
    // proximity: how clos is the object  7: close ; 0: far
    output reg [c_nb_prox-1:0] proximity_l, // left img
    output reg [c_nb_prox-1:0] proximity_r, // right img
    output reg [c_nb_prox-1:0] proximity,   // combined image 7:close, 0:far
    output reg [2:0] rgbfilter              // color filter used
  );
