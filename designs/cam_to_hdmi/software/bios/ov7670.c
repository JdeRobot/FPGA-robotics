...
int ret = 0;
	...
	while ( (vals->reg_num != 0xff || vals->value != 0xff) && (ret == 0) ) {
	    ...
	    debug_print("reset reg %02X, W(%02X) R(%02X)\n", vals->reg_num, vals->value, SCCB_Read(sensor->slv_addr, vals->reg_num) );
